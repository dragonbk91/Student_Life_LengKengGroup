...
public class Level_4_Kitchen_Scene extends ManageableLevel implements ILevel,IOnSceneTouchListener{
	 ...
	 final static private String TAG_KILLER = "killer";
	...
	public static AIPool killerPool;
	private TiledTextureRegion DialogTextureRegion;
	private TiledTextureRegion killer1TextureRegion;
	...
	public void loadResources(BaseGameActivity context) {			
		...
		killerPool = new AIPool(killer1TextureRegion);
		...
		LevelManager.countdown.animate(400, false, new IAnimationListener() {
			...
			public void onAnimationEnd(final AnimatedSprite pAnimatedSprite) {
				LevelManager.getEngine().runOnUpdateThread(new Runnable() {
					...
					public void run() {	
						// Touch Event		
						mScene.setOnSceneTouchListener(Level_4_Kitchen_Scene.this);						
						...
						// timer			
						timer.UpdateHandler();								
						// random killer's direction
						for (AI killer : killerPool.arrAI){
							killer.changeDirectionRanDom();
							killer.registerListener();
						}
	...
	public void loadEntity(final String type, final int pX, final int pY, final int velocity){
		// TODO loadEntity
				...
				} else if(type.equals(TAG_KILLER)){
					AI killer = killerPool.obtainPoolItem();
					if (!killer.isAttachToScene()) {
						mScene.attachChild(killer);
						killer.setAttachToScene(true);
					}
					killer.setPosition(Grid.COLUMN[pX] , Grid.ROW[pY]);
					killer.setVelocity(velocity);
					killer.unregisterListener();
	...
	public void loadBackground(){
		// TODO loadBackground			
		BitmapTextureAtlasTextureRegionFactory.setAssetBasePath("gfx/Level 4/");	
	...
	public void loadTexture(BaseGameActivity context){
		...
		killer1TextureRegion = BitmapTextureAtlasTextureRegionFactory.createTiledFromAsset(this.sheetBitmapTextureAtlas, context, "girl.png",1050,0,4,4);		
		DialogTextureRegion = BitmapTextureAtlasTextureRegionFactory.createTiledFromAsset(this.sheetBitmapTextureAtlas, context, "dialog.png",930,0,2,5);		
	...
	public void loadMusic(BaseGameActivity context) {
		// TODO loadMusic
		SoundFactory.setAssetBasePath("mfx/Level 4/");
        try {

        	soundDialog = SoundFactory.createSoundFromAsset(context.getEngine().getSoundManager(), context, "teacher_baoluc.ogg");    	
        ...
        MusicFactory.setAssetBasePath("mfx/Level 4/");
        try {
